...
void setup() {
  ...
  result = createImage(img.width, img.height, ALPHA); // create a new, initially transparent, ’result’ image
  noLoop();
}
...
void draw() {
  ...
  for (int i = 0; i < img.width * img.height; i++) {
    ...
    //if (j == 0 || j == 1 || j == img.height - 1 || j == img.height - 2 || (i == j*800) || (i == j*800 +1) || (i == (j+1)*800 - 1) || (i == (j+1)*800 - 2)) {
  ...
  for (int i = size -1; i >= 0; --i) {
    ...
    if (!graph.isConvex(c12, c23, c34, c41) || !graph.validArea(c12, c23, c34, c41, 2*area, area/5) || !graph.nonFlatQuad(c12, c23, c34, c41)) {
      quads.remove(i);
    }
  }
...
PImage gBlur(PImage img) {
  ...
  for (int i = 0; i < img.width * img.height; i++) {
    if (i%800 == 0) ++j;
    if (!(j == 0 || j == img.height - 1 || (i == j*800) || (i == (j+1)*800 -1))) {
      for (int k = 0; k < N; ++k) {
        for (int l = 0; l < N; ++l) {
          sum += kernel[k][l]*brightness(img.pixels[i + 800*(k - N/2) + (l - N/2)]);
        }
      }
...
PImage sobel(PImage img) {
  ...
  for (int i = 0; i < img.width * img.height; i++) {
    ...
    if (!(j == 0 || j == 1 || j == img.height - 1 || j == img.height - 2 || (i == j*800) || (i == j*800 +1) || (i == (j+1)*800 - 1) || (i == (j+1)*800 - 2))) {
      for (int k = 0; k < N; ++k) {
        for (int l = 0; l < N; ++l) {
          sum_h += hKernel[k][l]*brightness(img.pixels[i + 800*(k - N/2) + (l - N/2)]);
          sum_v += vKernel[k][l]*brightness(img.pixels[i + 800*(k - N/2) + (l - N/2)]);
        }
      }
